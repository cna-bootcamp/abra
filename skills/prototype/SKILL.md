...
- **MUST DO**:
  ...
  - 요청 의도 식별: 
    - 개선요청: 사용자 입력의 `"프로토타입 개선:"` 또는 `"워크플로우 개선:"` 이후 텍스트 추출
    - 에러수정: 사용자 입력의 `"프로토타입 에러:"` 또는 `"워크플로우 에러:"` 이후 텍스트 추출
    ※ Prefix가 없는 경우 요청 의도를 식별하여 개선요청이나 에러수정 요청으로 판단되면 사용자에게 확인하고 진행    
...
- 단, 총 소요 시간이 20분을 초과하면 즉시 중단하고 사용자에게 문제 보고
